...
def _get_outdated() -> list[dict]:
    ...
    try:
        ...
        log.warning("Konnte pip-outdated-Ausgabe nicht parsen.", exc_info=True)
        return []
...
def _get_installed() -> list[dict]:
    ...
    try:
        ...
        log.warning("Konnte pip-list-Ausgabe nicht parsen.", exc_info=True)
        return []
        ...
        if submitted:
            if not all([name, bank, typ, bank_account]):
                ...
                try:
                    ...
                    # nächste Import Datum/Betrag/Empfänger etc. stillschweigend verworfen hätte.
                    create_transaction_table(con, validated_iban)
    ...
    with st.expander("🗑️ Konto entfernen", expanded=False):
        if not saved:
            ...
            account_del = st.selectbox("Konto auswählen", saved, key="del_select",
                                       format_func=lambda k: saved_df.set_index("KEY").loc[k, "LABEL"])
            st.warning(
                f"⚠️ Löscht Keyring-Eintrag und Konto-Eintrag für **{account_del}**. "
                "Die bereits importierten Transaktionsdaten (Tabelle) bleiben erhalten, "
                "das Konto erscheint aber nicht mehr in den Auswahllisten."
            )
            if st.button("🗑️ Löschen", type="primary", width='stretch'):
                try:
                    delete_fints_credentials(account_del)
                    _del_row = saved_df[saved_df["KEY"] == account_del]
                    if not _del_row.empty:
                        con.execute(
                            "DELETE FROM Accounts WHERE IBAN = ?",
                            [_del_row.iloc[0]["IBAN"]],
                        )
                    st.session_state.pop("known_ibans_cache", None)
                    st.session_state.pop("saved_users_cache", None)
                    st.success(f"✅ Zugangsdaten und Konto-Eintrag für **{account_del}** gelöscht.")
                    st.rerun()
                except Exception as e:
